...
interface Tab {
  id: number;
  name: string;
  icon: React.ComponentType<any>; // Ajoutez une propriété pour l'icône
}
  ...
  return (
    <div className="mb-8">
      <div className="border-b border-gray-200">
        <nav className="flex justify-center -mb-px space-x-2 overflow-x-auto pb-1" aria-label="Tabs">
          {tabs.map((tab) => (
            <button
              key={tab.id}
            ...
            >
              <tab.icon className="h-5 w-5" /> {/* Affichez l'icône ici */}
              <span>{tab.name}</span>
            </button>
          ))}
        </nav>
